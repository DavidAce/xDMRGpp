...
namespace tools::finite::ops {
    ...
    template<typename Scalar> [[nodiscard]] extern std::optional<RealScalar<Scalar>>
                                                                         get_spin_component_along_axis    (StateFinite<Scalar>& state, std::string_view axis);
    template<typename Scalar>               extern void                  project_to_axis                  (StateFinite<Scalar>& state, const Eigen::Matrix2cd & paulimatrix, int sign, std::optional<svd::config> svd_cfg = std::nullopt);
    ...
    template<typename Scalar> [[nodiscard]] extern StateFinite<Scalar>   get_projection_to_axis           (const StateFinite<Scalar>& state, const Eigen::Matrix2cd & paulimatrix, int sign, std::optional<svd::config> svd_cfg = std::nullopt);
    ...
    template<typename T, typename Scalar> [[nodiscard]] extern Scalar    overlap                          (const StateFinite<Scalar>& state1, const StateFinite<Scalar>& state2);
    /* clang-format on */

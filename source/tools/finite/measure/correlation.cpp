...
Scalar tools::finite::measure::correlation(const StateFinite<Scalar> &state, const Eigen::Tensor<Scalar, 2> &op1, const Eigen::Tensor<Scalar, 2> &op2,
                                           long pos1, long pos2) {
    if(pos1 == pos2) {
        // Stack the operators
        Eigen::Tensor<Scalar, 2> op12 = op1.contract(op2, tenx::idx({0}, {1}));
        LocalObservableOp        ob12 = {op12, pos1};
        return expectation_value<Scalar>(state, std::vector{ob12});
    } else {
        // No need to stack
        LocalObservableOp ob1 = {op1, pos1};
        LocalObservableOp ob2 = {op2, pos2};
        return expectation_value<Scalar>(state, std::vector{ob1, ob2});
    }
}
template cx64  tools::finite::measure::correlation(const StateFinite<cx64> &state, const Eigen::Tensor<cx64, 2> &op1, const Eigen::Tensor<cx64, 2> &op2,
                                                   long pos1, long pos2);
template cx128 tools::finite::measure::correlation(const StateFinite<cx128> &state, const Eigen::Tensor<cx128, 2> &op1, const Eigen::Tensor<cx128, 2> &op2,
                                                   long pos1, long pos2);

template<typename Scalar>
Eigen::Tensor<Scalar, 2> tools::finite::measure::correlation_matrix(const StateFinite<Scalar> &state, const Eigen::Tensor<Scalar, 2> &op1,
                                                                    const Eigen::Tensor<Scalar, 2> &op2) {
    ...
    Eigen::Tensor<Scalar, 2> C(len, len);
    C.setZero();
...
template Eigen::Tensor<cx64, 2>  tools::finite::measure::correlation_matrix(const StateFinite<cx64> &state, const Eigen::Tensor<cx64, 2> &op1,
                                                                            const Eigen::Tensor<cx64, 2> &op2);
template Eigen::Tensor<cx128, 2> tools::finite::measure::correlation_matrix(const StateFinite<cx128> &state, const Eigen::Tensor<cx128, 2> &op1,
                                                                            const Eigen::Tensor<cx128, 2> &op2);
...
RealScalar<Scalar> tools::finite::measure::structure_factor(const StateFinite<Scalar> &state, const Eigen::Tensor<Scalar, 2> &correlation_matrix) {
    ...
    return tenx::MatrixMap(correlation_matrix).cwiseAbs2().colwise().sum().sum() / state.template get_length<double>();
}

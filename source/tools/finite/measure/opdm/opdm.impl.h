...
Eigen::Tensor<Scalar, 2> tools::finite::measure::opdm(const StateFinite<Scalar> &state, ModelType modelType) {
    if(auto cache = state.measurements.get_cached_opdm(state, modelType); cache) {
        if(not state.measurements.opdm) state.measurements.opdm = cache->value();
        return cache->value();
    }
    auto value = [&]() {
        switch(modelType) {
    ...
    state.measurements.set_cached_opdm(value, state, modelType);
    return value;
}
...
Eigen::Tensor<Scalar, 2> tools::finite::measure::opdm_general(const StateFinite<Scalar> &state) {
     ...
     *
     * where
     *      r++ = r++(i,j) = <c_i† c_j>  ⟨sp(i) sz(i)sz(i+1)...sz(j-1) sm(j)⟩
     *      r+- = r+-(i,j) = <c_i† c_j†> ⟨sp(i) sz(i)sz(i+1)...sz(j-1) sp(j)⟩
     *      r-+ = r-+(i,j) = <c_i  c_j>  ⟨sm(i) sz(i)sz(i+1)...sz(j-1) sm(j)⟩
     *      r-- = r+-(i,j) = <c_i  c_j†> ⟨sm(i) sz(i)sz(i+1)...sz(j-1) sp(j)⟩
     *
     ...
     *
     */
    tools::log->info("Measuring the one-particle density matrix (OPDM)");
    using RealT      = decltype(std::real(std::declval<Scalar>()));
    using MatrixType = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
    long       L     = state.template get_length<long>();
    MatrixType R     = MatrixType::Zero(2 * L, 2 * L); // Allocate the full rho matrix "R"
    ...
    auto make_cd = [&](long k) {
        opstring_t s;
        for(long x = 0; x < k; ++x) s.emplace_back(op_t{sz, x});
        s.emplace_back(op_t{sp, k});
        return s;
    };
    auto make_c_ = [&](long k) {
        opstring_t s;
        for(long x = 0; x < k; ++x) s.emplace_back(op_t{sz, x});
        s.emplace_back(op_t{sm, k});
        return s;
    };
    ...
    for(long pos_i = 0; pos_i < L; ++pos_i) {
        ...
        for(long pos_j = pos_i; pos_j < L; ++pos_j) {
            ...
            auto omp = c__i;
            auto omm = c__i;
            opp.insert(opp.end(), c__j.begin(), c__j.end());
            opm.insert(opm.end(), cd_j.begin(), cd_j.end());
            omp.insert(omp.end(), c__j.begin(), c__j.end());
            omm.insert(omm.end(), cd_j.begin(), cd_j.end());
    ...
    // tools::log->info("r++: trace {:.16f}\n{}", fp(rpp.trace()), linalg::matrix::to_string(rpp, 5));
    // tools::log->info("r+-: trace {:.16f}\n{}", fp(rpm.trace()), linalg::matrix::to_string(rpm, 5));
    ...
    assert(rmm.isApprox(r1pp, std::numeric_limits<RealT>::epsilon() * 1000));
...
Eigen::Tensor<Scalar, 2> tools::finite::measure::opdm_u1_sym(const StateFinite<Scalar> &state) {
     ...
     * Note that each r is an LxL matrix, so R must be a 2Lx2L matrix.
     * Note also that the signs are "crossed" intentionally: r++ has sp and sm, and so on.
     * Note! This version only works on states whose reduced density matrices decompose into diagonal
     * blocks with definite particle number (generated by U(1) symmetric Hamiltonians)
     */
    tools::log->info("Measuring the general one-particle density matrix (OPDM)");
    using MatrixType = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
    // using RealT      = decltype(std::real(std::declval<Scalar>()));
...
Eigen::Tensor<RealScalar<Scalar>, 1> tools::finite::measure::opdm_spectrum(const StateFinite<Scalar> &state, ModelType modelType) {
    if(auto cache = state.measurements.get_cached_opdm_spectrum(state, modelType); cache) {
        if(not state.measurements.opdm_spectrum) state.measurements.opdm_spectrum = cache->value();
        return cache->value();
    }
    auto opdm = tools::finite::measure::opdm(state, modelType);
    {
        ...
        if(maxEval > RealT{1} + 10 * eps or minEval < RealT{0} - 10 * eps)
            ...
            tools::log->debug("opdm_spectrum: {::.16f}", fv<RealT>(opdm_spectrum_vec));

        state.measurements.set_cached_opdm_spectrum(opdm_spectrum, state, modelType);
    }

...
namespace tools::finite::mps {
    ...
    template<typename CalcType, typename Scalar> [[nodiscard]] Eigen::Tensor<Scalar,1> mps2tensor   (const StateFinite<Scalar> & state);

    template<typename Scalar>  size_t move_center_point_single_site      (StateFinite<Scalar>& state, std::optional<svd::config> svd_cfg = std::nullopt);
    template<typename Scalar>  size_t move_center_point                  (StateFinite<Scalar>& state, std::optional<svd::config> svd_cfg = std::nullopt);
    template<typename Scalar>  size_t move_center_point_to_pos           (StateFinite<Scalar>& state, long pos, std::optional<svd::config> svd_cfg = std::nullopt);
    template<typename Scalar>  size_t move_center_point_to_pos_dir       (StateFinite<Scalar>& state, long pos, int dir, std::optional<svd::config> svd_cfg = std::nullopt);
    template<typename Scalar>  size_t move_center_point_to_inward_edge   (StateFinite<Scalar>& state, std::optional<svd::config> svd_cfg = std::nullopt);
    ...
    template<typename Scalar>  void truncate_next_sites                  (StateFinite<Scalar>& state, size_t num_sites = 4, std::optional<svd::config> svd_cfg = std::nullopt);

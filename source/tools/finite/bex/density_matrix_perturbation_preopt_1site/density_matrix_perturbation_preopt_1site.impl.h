...
BondExpansionResult<Scalar> tools::finite::bex::density_matrix_perturbation_preopt_1site(TensorsFinite<Scalar> &tensors, BondExpansionConfig bcfg) {
    ...
    if(mpsP.dimensions()[0] * std::min(mpsP.dimensions()[1], mpsP.dimensions()[2]) < std::max(mpsP.dimensions()[1], mpsP.dimensions()[2]))
        tools::log->warn("density_matrix_perturbation_preopt_1site: Bond expansion failed: {} -> {}", dimP_old, fmw::wrap(mpsP.dimensions()));
    ...
    if(mpsL.get_chiR() > bcfg.bond_lim) {
        throw except::logic_error("density_matrix_perturbation_preopt_1site: {}{} - {}{} | bond {} > max bond{}", mpsL.get_tag(),

...
BondExpansionResult<Scalar> tools::finite::bex::expand_bond_dmrg3s(StateFinite<Scalar> &state, const ModelFinite<Scalar> &model, EdgesFinite<Scalar> &edges,
    ...
    if(not num::all_equal(state.active_sites, model.active_sites, edges.active_sites))
        throw except::runtime_error("expand_bond_dmrg3s: All active sites are not equal: state {} | model {} | edges {}", fmw::wrap(state.active_sites),

...
bool tools::finite::mps::normalize_state(StateFinite<Scalar> &state, std::optional<svd::config> svd_cfg, NormPolicy norm_policy) {
    ...
    if(tools::log->level() <= spdlog::level::debug)
        tools::log->debug("normalize_state: {} old local norm = {:.16f} | pos {} | dir {} | bond dims {}", enum2sv(norm_policy),
                          tools::finite::measure::norm_state(state), pos, dir, tools::finite::measure::bond_dimensions(state));
    ...
    for(int trial = 0; trial < maxtrials; ++trial) {
        if constexpr(settings::debug_normalization) {
            ...
            tools::log->debug("Normalization trial {}/{} | current state norm {:.16f}", trial, maxtrials, norm);
        }
    ...
    if(not state.is_normalized_on_all_sites(normErrorTolerance)) {
        ...
        for(const auto &mps : state.mps_sites) {
            tools::log->warn("{} | norm  err {:.16f} | is_normalized {:<7} | L norm {:.16f}", mps->get_tag(), mps->get_norm_error(),
                             mps->is_normalized(normErrorTolerance), tenx::VectorMap(mps->get_L()).norm());
    ...
    if(tools::log->level() <= spdlog::level::debug) {
        auto norm = tools::finite::measure::norm_state(state);
        tools::log->debug("normalize_state: new local norm = {:.16f} | pos {} | dir {} | bond dims {}", norm, pos, dir,

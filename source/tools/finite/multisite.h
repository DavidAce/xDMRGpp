...
namespace tools::finite::multisite {
    ...
    template<typename Scalar> extern std::array<long, 4> get_dimensions_squared(const ModelFinite<Scalar> &model, std::optional<std::vector<size_t>> active_sites = std::nullopt);

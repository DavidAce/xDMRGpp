...
namespace tools::finite::multisite {
    /* clang-format off */
    template<typename Scalar>  long                get_problem_size(const StateFinite<Scalar> &state, std::optional<std::vector<size_t>> sites = std::nullopt);
    template<typename Scalar>  std::array<long, 3> get_dimensions(const StateFinite<Scalar> &state, std::optional<std::vector<size_t>> sites = std::nullopt);

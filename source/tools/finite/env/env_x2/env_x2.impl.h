...
void tools::finite::env::rebuild_edges_ene_x2(const StateFinite<Scalar> &state, const ModelFinite<Scalar> &model, EdgesFinite<Scalar> &edges) {
    ...
    for(size_t pos = min_pos; pos < stopL; pos++) {
        ...
        if(refresh) {
            ...
            env_rght.set_block(new_x2.to_TensorType(), env_here, mps, mpo);
            old_x2 = new_x2;
        } else {
            old_x2 = env_rght.get_block();
            old_x2.renorm();
        }
    ...
    for(size_t pos = max_pos; pos > stopR; --pos) {
        ...
        if(refresh) {
            ...
            env_left.set_block(new_x2.to_TensorType(), env_here, mps, mpo);
            old_x2 = new_x2;
        } else {
            old_x2 = env_left.get_block();
            old_x2.renorm();
        }

...
BondExpansionResult<Scalar> tools::finite::env::density_matrix_perturbation_postopt_1site(TensorsFinite<Scalar> &tensors, BondExpansionConfig bcfg) {
    ...
    if(not num::all_equal(state.get_length(), model.get_length(), edges.get_length()))
        throw except::runtime_error("density_matrix_perturbation_postopt_1site: All lengths not equal: state {} | model {} | edges {}", state.get_length(),
    ...
    if(state.get_direction() > 0) {
        ...
        tools::log->debug("Bond expansion r2l {} | {} | χmax {} | χ {} -> {} -> {}", pos_expanded, flag2str(bcfg.policy), bcfg.bond_lim, dimP_old,
                          M.dimensions(), M_P.dimensions());

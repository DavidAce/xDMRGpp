...
struct BondExpansionResult {
    ...
    double                           bondexp_factor = std::numeric_limits<double>::quiet_NaN(); /*!< The mixing factor used in the expansion */
    ...
    Eigen::Tensor<Scalar, 3>         mixed_blk;                                   /*!< The mixed mps block */
    std::optional<std::reference_wrapper<EnvEne<Scalar>>> env_ene = std::nullopt; /*!< Reference to the ene environment (either L or R) */
    std::optional<std::reference_wrapper<EnvVar<Scalar>>> env_var = std::nullopt; /*!< Reference to the var environment (either L or R) */
    std::string                                           msg;
};
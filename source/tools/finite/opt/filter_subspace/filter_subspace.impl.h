...
void tools::finite::opt::internal::subspace::filter_subspace(std::vector<opt_mps<Scalar>> &subspace, size_t max_accept) {
    if(subspace.empty()) throw std::runtime_error("filter_subspace: empty subspace");
    ...
    if(subspace_errors.empty()) throw std::runtime_error("filter_subspace: no basis vectors");
    ...
    RealScalar<Scalar> subspace_error_final = subspace_errors.empty() ? RealScalar<Scalar>{1} : subspace_errors.back();
    size_t             idx                  = 0;

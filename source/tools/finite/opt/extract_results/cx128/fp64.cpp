...
template void tools::finite::opt::internal::extract_results<T>(const TensorsFinite<Scalar> &tensors, const opt_mps<Scalar> &initial, const OptMeta &opt_meta, const solver_gdplusk<T> &solver, std::vector<opt_mps<Scalar>> &results);

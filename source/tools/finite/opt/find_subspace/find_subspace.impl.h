...
std::vector<opt_mps<Scalar>> subspace::find_subspace(const TensorsFinite<Scalar> &tensors, const OptMeta &meta, reports::subs_log<Scalar> &slog) {
    ...
    if constexpr(sfinae::is_std_complex_v<T>) {
        ...
        if(num::gt(trunc, 1e-12)) tools::log->warn("truncating imag of eigvecs, sum: {:.3e}", trunc);
    ...
    if(primme->monitor != nullptr and *rNorm != 0) {
        ...
        if(check_subspace) {
            ...
            if(valid_cols.size() >= 2) {
                ...
                R sq_sum_overlap            = overlaps.cwiseAbs2().sum();
                R subspace_error            = R{1} - sq_sum_overlap;
                if(num::gt(max_overlap, 1.0 + 1e-6)) tools::log->debug("max_overlap larger than one: {:.16f}", max_overlap);
                if(num::gt(sq_sum_overlap, 1.0 + 1e-6)) tools::log->debug("eps larger than one: {:.16f}", sq_sum_overlap);
                if(num::lt(min_overlap, 0.0)) tools::log->debug("min_overlap smaller than zero: {:.16f}", min_overlap);

...
void tools::infinite::h5::save::bonds(h5pp::File &h5file, const StorageInfo &sinfo, const StateInfinite<Scalar> &state) {
...
template<typename Scalar>
void tools::infinite::h5::save::state(h5pp::File &h5file, const StorageInfo &sinfo, const StateInfinite<Scalar> &state) {
    // Checks if the current entry has already been saved
    // If it is empty because we are resuming, check if there is a log entry on file already
    auto tic        = tid::tic_token("state");
...
template<typename Scalar>
void tools::infinite::h5::save::edges(h5pp::File &h5file, const StorageInfo &sinfo, const EdgesInfinite<Scalar> &edges) {
    auto        tic = tid::tic_token("edges");
    const auto &ene = edges.get_env_ene_block();
    const auto &var = edges.get_env_var_block();
...
void tools::infinite::h5::save::model(h5pp::File &h5file, const StorageInfo &sinfo, const ModelInfinite<Scalar> &model) {
...
template<typename Scalar>
void tools::infinite::h5::save::mpo(h5pp::File &h5file, const StorageInfo &sinfo, const ModelInfinite<Scalar> &model) {

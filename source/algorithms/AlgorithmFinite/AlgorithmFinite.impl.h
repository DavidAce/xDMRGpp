...
void AlgorithmFinite<Scalar>::update_truncation_error_limit() {
    ...
    //             tools::log->info("update_truncation_error_limit: Trial {} trnc_lim {:.3e} var {:.8e} (lim: {:.8e}): bond dims: {}", trials, trnc_temp,
    //                              fp(evar_new), fp(evar_lim), tools::finite::measure::bond_dimensions(tensors.get_state()));
    //
    ...
    if(drop_if_saturated) condition.emplace_back("if_saturated");
    if(drop_if_has_stuck) condition.emplace_back("if_has_stuck");
    if(drop_if_next_iter) condition.emplace_back("if_next_iter");
    if(drop_if_even_iter) condition.emplace_back("if_even_iter");
...
void AlgorithmFinite<Scalar>::update_eigs_tolerance() {
    ...
    if(drop_if_stk_algo and is_stk_algo) reason.emplace_back("stuck");          // Can only go down
    if(drop_if_next_iter and is_next_iter) reason.emplace_back("next iter");    // Can only go down
...
AlgorithmFinite<Scalar>::log_entry::log_entry(const AlgorithmStatus &s, const TensorsFinite<Scalar> &t)
    ...
    //     StateFinite<Scalar> tmp_state    = t.get_state();
    //     auto                L            = t.template get_length<RealScalar>();
    //     auto                mpos_shifted = t.get_model().get_mpo_tensors(energy / L, MposWithEdges::ON, MpoCompress::DPL);
    //     auto                svdcfg       = svd::config(status.bond_max, status.trnc_min);
    ...
    //     StateFinite<Scalar> tmp_state    = t.get_state();
    //     auto                L            = t.template get_length<RealScalar>();
    //     auto                mpos_shifted = t.get_model().get_mpo_tensors(E1_global1 / L, MposWithEdges::ON, MpoCompress::DPL);
    //     auto                svdcfg       = svd::config(status.bond_max, status.trnc_min);
    //     tools::finite::ops::apply_mpos_general(tmp_state, mpos_shifted, svdcfg);
    //     energy_variance_global = std::real(tools::finite::ops::overlap<Scalar>(tmp_state, tmp_state));
...
void AlgorithmFinite<Scalar>::check_convergence_energy(std::optional<RealScalar> saturation_sensitivity) {
    ...
    auto report = check_saturation(energy_iter, saturation_sensitivity.value(), SaturationPolicy::val | SaturationPolicy::mid);

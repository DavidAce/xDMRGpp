...
EnvVar<Scalar> EnvVar<Scalar>::enlarge(const MpsSite<Scalar> &mps, const MpoSite<Scalar> &mpo) const {
    if constexpr(settings::debug_edges_var) tools::log->trace("EnvVar<Scalar>::enlarge(mps,mpo): {}{}[{}]", tag, side, get_position());
    ...
    env.build_blkx2(this->get_blkx2(), mps.template get_M_bare_as<Scalar>(), mpo.template MPO2_as<Scalar>());
    ...
    //     return env;
    // }
    // env.enlarge(mps.template get_M_bare_as<Scalar>(), mpo.template MPO2_as<Scalar>()); // calls base
    ...
    // env.unique_id_mps = mps.get_unique_id();
    // env.unique_id_mpo = mpo.get_unique_id_sq();
    if constexpr(settings::debug_edges_var) {
        tools::log->trace("EnvVar::enlarge(mps,mpo): side({}), pos({}): unique_id_env: {}", side, get_position(), env.unique_id_env.value());
        tools::log->trace("EnvVar::enlarge(mps,mpo): side({}), pos({}): unique_id_mps: {}", side, get_position(), env.unique_id_mps.value());
        tools::log->trace("EnvVar::enlarge(mps,mpo): side({}), pos({}): unique_id_mpo: {}", side, get_position(), env.unique_id_mpo.value());
    }
    return env;
}
...
void EnvVar<Scalar>::refresh(const EnvVar &env, const MpsSite<Scalar> &mps, const MpoSite<Scalar> &mpo) {
    ...
    if constexpr(settings::debug)
        if(not num::all_equal(env.get_position(), mps.get_position(), mpo.get_position()))
            throw except::logic_error("EnvVar::enlarge(): side({}), pos({}),: All positions are not equal: env {} | mps {} | mpo {}", side, get_position(),
                                      get_position(), mps.get_position(), mpo.get_position());
    ...
    if(not has_block()) {
        if constexpr(settings::debug_edges_var) tools::log->trace("Refreshing {} env{}({}): missing block", tag, side, get_position());
    ...
    if(refresh) {
        ...
        build_blkx2(*env.blkx2, mps.template get_M_bare_as<Scalar>(), mpo.template MPO2_as<Scalar>());
...
template<typename Scalar>
void EnvVar<Scalar>::set_block(const Eigen::Tensor<Scalar, 3> &blk, const EnvVar &env, const MpsSite<Scalar> &mps, const MpoSite<Scalar> &mpo) {
    // If side == L, env,mps and mpo are all corresponding to the neighbor on the left
    // If side == R, env,mps and mpo are all corresponding to the neighbor on the right
    if constexpr(settings::debug)
        if(not num::all_equal(env.get_position(), mps.get_position(), mpo.get_position()))

...
class MpoSite {
    ...
    Scalar                                  energy_shift_mpo       = Scalar(0.0, 0.0); /*!< Energy shift for this mpo (to make energy-shifted MPO views) */
    ...
    Eigen::Tensor<cx128, 4>                 mpo_internal_q;
    ...
    virtual Eigen::Tensor<cx128, 4>  get_mpo_q(Scalar energy_shift_per_site, std::optional<std::vector<size_t>> nbody = std::nullopt,
    ...
    [[nodiscard]] const Eigen::Tensor<cx128, 4>  &MPO_q() const;
    [[nodiscard]] Eigen::Tensor<Scalar, 4>        MPO_energy_shifted_view(Scalar energy_shift_per_site) const;
    [[nodiscard]] Eigen::Tensor<cx128, 4>         MPO_energy_shifted_view_q(Scalar energy_shift_per_site) const;
    [[nodiscard]] Eigen::Tensor<Scalar, 4>        MPO_nbody_view(std::optional<std::vector<size_t>> nbody,
                                                                 std::optional<std::vector<size_t>> skip = std::nullopt) const;
    [[nodiscard]] Eigen::Tensor<cx128, 4>         MPO_nbody_view_q(std::optional<std::vector<size_t>> nbody,
                                                                   std::optional<std::vector<size_t>> skip = std::nullopt) const;
    ...
    [[nodiscard]] Eigen::Tensor<Scalar, 4>        MPO2_nbody_view(std::optional<std::vector<size_t>> nbody,
                                                                  std::optional<std::vector<size_t>> skip = std::nullopt) const;
    /* clang-format off */
    // template<typename T> [[nodiscard]] Eigen::Tensor<T, 4>  MPO_as() const;
    template<typename T>
    [[nodiscard]] decltype(auto) MPO_as() const {
        if constexpr(tenx::sfinae::is_quadruple_prec_v<T>) return tenx::asScalarType<T>(MPO_q());
        else return tenx::asScalarType<T>(MPO());
    }
    template<typename T> [[nodiscard]] Eigen::Tensor<T, 4>  MPO_energy_shifted_view_as(Scalar energy_shift_per_site) const;
    template<typename T> [[nodiscard]] Eigen::Tensor<T, 4>  MPO_nbody_view_as(std::optional<std::vector<size_t>> nbody,
                                                                                        std::optional<std::vector<size_t>> skip = std::nullopt) const;

    // template<typename T> [[nodiscard]] Eigen::Tensor<T, 4>  MPO2_as() const;
    template<typename T>
    [[nodiscard]] decltype(auto) MPO2_as() const {
        return tenx::asScalarType<T>(MPO2());
    }
    template<typename T> [[nodiscard]] Eigen::Tensor<T, 4>  MPO2_nbody_view_as(std::optional<std::vector<size_t>> nbody,
                                                                                         std::optional<std::vector<size_t>> skip = std::nullopt) const;
    /* clang-format on */

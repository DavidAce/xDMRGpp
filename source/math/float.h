...
namespace sfinae {
    template<typename T>
    using Real = decltype(std::real(std::declval<std::remove_cvref_t<T>>()));
    ...
    template<typename T>
    concept is_single_prec_v = std::same_as<Real<T>, fp32>;

    template<typename T>
    concept is_double_prec_v = std::same_as<Real<T>, fp64>;

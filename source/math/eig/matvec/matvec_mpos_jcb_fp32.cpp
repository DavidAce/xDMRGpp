...
#include "matvec_mpos_jcb_fp32.h"
#include "../log.h"
...
MatVecMPOS_JCB_FP32<T>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, /*!< The Hamiltonian MPO's  */
...
template MatVecMPOS_JCB_FP32<cx64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvEne &> &envs_);
template MatVecMPOS_JCB_FP32<fp64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvEne &> &envs_);
template MatVecMPOS_JCB_FP32<cx64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvVar &> &envs_);
template MatVecMPOS_JCB_FP32<fp64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvVar &> &envs_);
...
MatVecMPOS_JCB_FP32<T>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, /*!< The Hamiltonian MPO's  */
    ...
    : MatVecMPOS_JCB_FP32(mpos_, enva_) {
...
template MatVecMPOS_JCB_FP32<fp64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvVar &> &enva_,
                                                        const env_pair<const EnvEne &> &envb);
template MatVecMPOS_JCB_FP32<cx64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvVar &> &enva_,
                                                        const env_pair<const EnvEne &> &envb);

template MatVecMPOS_JCB_FP32<fp64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvEne &> &enva_,
                                                        const env_pair<const EnvVar &> &envb);
template MatVecMPOS_JCB_FP32<cx64>::MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos_, const env_pair<const EnvEne &> &enva_,
                                                        const env_pair<const EnvVar &> &envb);

template<typename T>
int MatVecMPOS_JCB_FP32<T>::rows() const {
    return safe_cast<int>(size_mps);
}

template<typename T>
int MatVecMPOS_JCB_FP32<T>::cols() const {
    return safe_cast<int>(size_mps);
}
...
//
// template<typename T>
// void MatVecMPOS_JCB_FP32<T>::thomas(long size, T *x, T *const dl, T *const dm, T *const du) {
//     /*
...
//
// template<typename T>
// void MatVecMPOS_JCB_FP32<T>::thomas2(long size, T *x, T *const a, T *const b, T *const c) {
//     /*
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::T32 MatVecMPOS_JCB_FP32<T>::get_matrix_element(long I, long J, const std::vector<Eigen::Tensor<T, 4>> &MPOS,
                                                                                const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const {
    if(I < 0 or I >= size_mps) return 0;
    if(J < 0 or J >= size_mps) return 0;

    if(MPOS.empty()) {
        return I == J ? T32(1.0) : T32(0.0); // Assume an identity matrix
    }
    ...
    auto mpo_i = Eigen::Tensor<T, 4>();
    auto temp  = Eigen::Tensor<T, 4>();
    ...
    if(fullsystem and mpo_i.size() == 1) {
        ...
        // return static_cast<T32>(mpo_i.coeff(0));
        T result = mpo_i.coeff(0);
        if constexpr(std::is_same_v<T, std::complex<fp64>>) {
            return std::complex(static_cast<fp32>(result.real()), static_cast<fp32>(result.imag()));
        } else
            return static_cast<T32>(result);
    }
    ...
    T    result     = envL_j_map.transpose() * mpo_i_map * envR_k_map;
    if constexpr(std::is_same_v<T, cx64>) {
        return std::complex(static_cast<fp32>(result.real()), static_cast<fp32>(result.imag()));
    } else
        return static_cast<T32>(result);
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::VectorT32 MatVecMPOS_JCB_FP32<T>::get_diagonal_new(long offset, const std::vector<Eigen::Tensor<T, 4>> &MPOS,
                                                                                    const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const {
    if(MPOS.empty()) return VectorT32::Ones(size_mps); // Assume an identity matrix
    auto res = VectorT32(size_mps);
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixT32
    MatVecMPOS_JCB_FP32<T>::get_diagonal_block_old(long offset, long extent, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                                                   const Eigen::Tensor<T, 3> &ENVR) const {
    if(MPOS.empty()) return MatrixT32::Identity(extent, extent);
    extent = std::min(extent, size_mps - offset);
    if(offset >= size_mps) { return {}; }
    auto res = MatrixT32(extent, extent);
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixT32 MatVecMPOS_JCB_FP32<T>::get_diagonal_block(long offset, long extent, const std::vector<Eigen::Tensor<T, 4>> &MPOS,
                                                                                      const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const {
    if(MPOS.empty()) return MatrixT32::Identity(extent, extent);
    extent = std::min(extent, size_mps - offset);
    if(offset >= size_mps) { return {}; }
...
#pragma omp parallel for collapse(2)
        ...
        while(JY <= JN) {
            ...
            resC.block(off_resC[0], off_resC[1], ext_resC[0], ext_resC[1]) =
                Eigen::Map<MatrixType>(blockC.data(), blockC.dimension(0), blockC.dimension(1)).block(I0 - R0, JY - C0, ext_resC[0], ext_resC[1]).template cast<T32>();
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixT32 MatVecMPOS_JCB_FP32<T>::get_diagonal_block(
    long offset, long extent, T32 shift, const std::vector<Eigen::Tensor<T, 4>> &MPOS_A, const Eigen::Tensor<T, 3> &ENVL_A, const Eigen::Tensor<T, 3> &ENVR_A,
    const std::vector<Eigen::Tensor<T, 4>> &MPOS_B, const Eigen::Tensor<T, 3> &ENVL_B, const Eigen::Tensor<T, 3> &ENVR_B) const {
    if(MPOS_A.empty()) return MatrixT32::Identity(extent, extent);
    extent = std::min(extent, size_mps - offset);
    if(offset >= size_mps) { return {}; }
    auto  t_old   = tid::ur("old");
    auto  t_new   = tid::ur("new");
...

    {
        ...
        while(JY <= JN) {
            ...
            auto get_subblock = [&](const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                                    const Eigen::Tensor<T, 3> &ENVR) -> MatrixT32 {
                if(MPOS.empty()) {
                    auto subblock = MatrixT32(ext_res[0], ext_res[1]);
                    subblock.setZero();
                ...
                return Eigen::Map<MatrixType>(block.data(), ext_blk2[0], ext_blk2[1]).block(IX - R0, JY - C0, ext_res[0], ext_res[1]).template cast<T32>();
            };
            ...
            MatrixT32 blkres = res.block(off_res[0], off_res[1], ext_res[0], ext_res[1]);
            // MatrixT32 blkdbg = dbg.block(off_res[0], off_res[1], ext_res[0], ext_res[1]);
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixT32 MatVecMPOS_JCB_FP32<T>::get_diagonal_block_old(
    long offset, long extent, T32 shift, const std::vector<Eigen::Tensor<T, 4>> &MPOS_A, const Eigen::Tensor<T, 3> &ENVL_A, const Eigen::Tensor<T, 3> &ENVR_A,
    const std::vector<Eigen::Tensor<T, 4>> &MPOS_B, const Eigen::Tensor<T, 3> &ENVL_B, const Eigen::Tensor<T, 3> &ENVR_B) const {
    if(MPOS_A.empty()) return MatrixT32::Identity(extent, extent);
    extent = std::min(extent, size_mps - offset);
...
#pragma omp parallel for collapse(2)
    ...
    {
        // auto dbg = MatrixT32(extent, extent);
        // #pragma omp parallel for collapse(2)
        // for(long J = 0; J < extent; J++) {
        //     for(long I = J; I < extent; I++) {
        //         if(I + offset >= size_mps) continue;
        ...
        while(JY <= JN) {
            ...
            auto get_tile2 = [&](const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) -> MatrixT32 {
                if(MPOS.empty()) {
                    auto block = MatrixT32(ext_res[0], ext_res[1]);
                    block.setZero();
                    // Set 1's along the diagonal of the supermatrix
                ...
                block.device(*threads->dev) = ENVL.slice(off_envl, ext_envl)
                                                  ...
                                                  .shuffle(tenx::array6{2, 0, 4, 3, 1, 5})
                                                  .reshape(ext_blk2);
                return Eigen::Map<MatrixType>(block.data(), ext_blk2[0], ext_blk2[1]).block(I0 - R0, JY - C0, ext_res[0], ext_res[1]).template cast<T32>();
            };
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::VectorT32 MatVecMPOS_JCB_FP32<T>::get_row(long row_idx, const std::vector<Eigen::Tensor<T, 4>> &MPOS,
                                                                           const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const {
    auto res = VectorT32(size_mps);
#pragma omp parallel for
...
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::VectorT32 MatVecMPOS_JCB_FP32<T>::get_col(long col_idx, const std::vector<Eigen::Tensor<T, 4>> &MPOS,
                                                                           const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const {
    auto res = VectorT32(size_mps);
#pragma omp parallel for
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::FactorOP() {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultOPv([[maybe_unused]] T *mps_in_, [[maybe_unused]] T *mps_out_) {
    throw except::runtime_error("void MatVecMPOS_JCB_FP32<T>::MultOPv(...) not implemented");
}

// template<typename T>
// void MatVecMPOS_JCB_FP32<T>::MultOPv([[maybe_unused]] void *x, [[maybe_unused]] int *ldx, [[maybe_unused]] void *y, [[maybe_unused]] int *ldy,
//                             [[maybe_unused]] int *blockSize, [[maybe_unused]] primme_params *primme, [[maybe_unused]] int *err) {
//     throw except::runtime_error("MatVecMPOS_JCB_FP32<T>::MultOPv(...): not implemented");
// }
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultOPv(void *x, int *ldx, void *y, int *ldy, int *blockSize, [[maybe_unused]] primme_params *primme, int *err) {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultAx(T *mps_in_, T *mps_out_) {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultAx(void *x, int *ldx, void *y, int *ldy, int *blockSize, [[maybe_unused]] primme_params *primme, [[maybe_unused]] int *err) {
    for(int i = 0; i < *blockSize; i++) {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultBx(T *mps_in_, T *mps_out_) {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultBx(void *x, int *ldx, void *y, int *ldy, int *blockSize, [[maybe_unused]] primme_params *primme, [[maybe_unused]] int *err) {
    for(int i = 0; i < *blockSize; i++) {
        T *mps_in_ptr  = static_cast<T *>(x) + *ldx * i;
        T *mps_out_ptr = static_cast<T *>(y) + *ldy * i;
...
// template<typename T>
// std::vector<std::pair<T, long>> MatVecMPOS_JCB_FP32<T>::get_k_smallest(const VectorType &vec, size_t k) const {
...
// template<typename T>
// std::vector<long> MatVecMPOS_JCB_FP32<T>::get_k_smallest(const VectorType &vec, size_t k) const {
//     std::priority_queue<T> pq;
//     for(auto d : vec) {
//         if(pq.size() >= k && pq.top() > d) {
//             pq.push(d);
...
//
// template<typename T>
// std::vector<long> MatVecMPOS_JCB_FP32<T>::get_k_largest(const VectorType &vec, size_t k) const {
//     using idx_pair = std::pair<Scalar, long>;
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::CalcPc(T32 shift) {
    if(readyCalcPc) return;
    if(preconditioner == eig::Preconditioner::NONE) {
        eig::log->info("MatVecMPOS_JCB_FP32<T>::CalcPc(): no preconditioner chosen");
        return;
    }
    ...
    if(jcbBlockSize == 1) {
        eig::log->trace("MatVecMPOS_JCB_FP32<T>::CalcPc(): calculating the jacobi preconditioner ... (shift = {:.16f})", shift);
        jcbDiagA = get_diagonal_new(0, mpos_A, envL_A, envR_A);
        jcbDiagB = get_diagonal_new(0, mpos_B, envL_B, envR_B);
        eig::log->debug("MatVecMPOS_JCB_FP32<T>::CalcPc(): calculating the jacobi preconditioner ... done (shift = {:.16f})", shift);
    } else if(jcbBlockSize > 1) {
        long nblocks = 1 + ((size_mps - 1) / jcbBlockSize); // ceil: note that the last block may be smaller than blocksize!

        eig::log->debug("MatVecMPOS_JCB_FP32<T>::CalcPc(): calculating the block jacobi preconditioner | {} | size {} | diagonal blocksize {} | nblocks {} ...",
        ...
        for(long blkidx = 0; blkidx < nblocks; ++blkidx) {
            ...
            t_dblk.tic();
            MatrixT32 blockI = get_diagonal_block(offset, extent, shift, mpos_A, envL_A, envR_A, mpos_B, envL_B, envR_B);
            t_dblk.toc();
            ...
            // MatrixT32 blockIdbg = blockA - blockB * shift; // Reset
            //
            ...
            auto sparseRM     = std::make_unique<SparseT32R>();
            auto sparseCM     = std::make_unique<SparseT32>();
            auto blockPtr     = std::make_unique<MatrixT32>();
            ...
            switch(factorization) {
                case eig::Factorization::NONE: {
                    eig::log->warn("MatVecMPOS_JCB_FP32::CalcPc(): No factorization has been set for the preconditioner");
                    break;
                }
                ...
                case eig::Factorization::QR: {
                    auto qr   = Eigen::HouseholderQR<MatrixT32>(blockI);
                    blockI    = qr.solve(MatrixT32::Identity(extent, extent));
                    qrSuccess = true;
                    break;
                }
                case eig::Factorization::ILUT: {
                    ...
                    *sparseRM = blockI.sparseView(1e-1, stdv);
                    ...
                    if(!bicgSuccess) {
                        ...
                        sparseRM->resize(0, 0);
                        blockI = MatrixT32(blockI.diagonal().cwiseInverse().asDiagonal());
                        break;
                    }
                    break;
                }
            ...
            if(!lltSuccess and !ldltSuccess and !luSuccess and !qrSuccess and factorization != eig::Factorization::ILUT and
               factorization != eig::Factorization::ILDLT) {
                ...
                blockI       = Eigen::ColPivHouseholderQR<MatrixT32>(blockI).inverse(); // Should work on any matrix
                solveSuccess = true;
            }
#pragma omp ordered
            {
        ...
        eig::log->debug("MatVecMPOS_JCB_FP32<T>::CalcPc(): calculating the block jacobi preconditioner | size {} | diagonal blocksize {} | nblocks {} ... done "
                        "| t {:.3e} s | avg "
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultPc([[maybe_unused]] void *x, [[maybe_unused]] int *ldx, [[maybe_unused]] void *y, [[maybe_unused]] int *ldy,
                                    [[maybe_unused]] int *blockSize, [[maybe_unused]] primme_params *primme, [[maybe_unused]] int *err) {
    for(int i = 0; i < *blockSize; i++) {
        T32 shift = static_cast<fp32>(primme->ShiftsForPreconditioner[i]);
        if(!mpos_B.empty())
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::MultPc([[maybe_unused]] T *mps_in_, [[maybe_unused]] T *mps_out_, T32 shift) {
    ...
    if(jcbMaxBlockSize == 1) {
        ...
        mps_out          = denseJcbDiagonal.template cast<T64>().array().cwiseInverse().cwiseProduct(mps_in.array());
        num_pc++;
...
#pragma omp parallel for
        for(size_t idx = 0; idx < denseJcbBlocks.size(); ++idx) {
            ...
            mps_out.segment(offset, extent).noalias() = block.template cast<T64>().template selfadjointView<Eigen::Lower>() * mps_in.segment(offset, extent);
        }
#pragma omp parallel for
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::print() const {}

template<typename T>
void MatVecMPOS_JCB_FP32<T>::reset() {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::set_shift(std::complex<double> shift) {
...
template<typename T>
void MatVecMPOS_JCB_FP32<T>::set_mode(const eig::Form form_) {
    form = form_;
}
template<typename T>
void MatVecMPOS_JCB_FP32<T>::set_side(const eig::Side side_) {
    side = side_;
}
template<typename T>
void MatVecMPOS_JCB_FP32<T>::set_jcbMaxBlockSize(std::optional<long> size) {
    if(size.has_value()) {
...
template<typename T>
T MatVecMPOS_JCB_FP32<T>::get_shift() const {
    if constexpr(std::is_same_v<T, fp64>)
...
template<typename T>
eig::Form MatVecMPOS_JCB_FP32<T>::get_form() const {
    return form;
}
template<typename T>
eig::Side MatVecMPOS_JCB_FP32<T>::get_side() const {
    return side;
}
template<typename T>
eig::Type MatVecMPOS_JCB_FP32<T>::get_type() const {
    if constexpr(std::is_same_v<T, fp64>)
...
template<typename T>
const std::vector<Eigen::Tensor<T, 4>> &MatVecMPOS_JCB_FP32<T>::get_mpos() const {
    return mpos_A;
}
template<typename T>
const Eigen::Tensor<T, 3> &MatVecMPOS_JCB_FP32<T>::get_envL() const {
    return envL_A;
}
template<typename T>
const Eigen::Tensor<T, 3> &MatVecMPOS_JCB_FP32<T>::get_envR() const {
    return envR_A;
}

template<typename T>
long MatVecMPOS_JCB_FP32<T>::get_size() const {
    return size_mps;
}

template<typename T>
std::array<long, 3> MatVecMPOS_JCB_FP32<T>::get_shape_mps() const {
    return shape_mps;
}
template<typename T>
std::vector<std::array<long, 4>> MatVecMPOS_JCB_FP32<T>::get_shape_mpo() const {
...
template<typename T>
std::array<long, 3> MatVecMPOS_JCB_FP32<T>::get_shape_envL() const {
    return envL_A.dimensions();
}
template<typename T>
std::array<long, 3> MatVecMPOS_JCB_FP32<T>::get_shape_envR() const {
    return envR_A.dimensions();
}

template<typename T>
Eigen::Tensor<T, 6> MatVecMPOS_JCB_FP32<T>::get_tensor() const {
    if(mpos_A.size() == 1) {
    ...
    throw std::runtime_error("MatVecMPOS_JCB_FP32<T>::get_tensor(): Not implemented for mpos.size() > 1");
}
template<typename T>
Eigen::Tensor<T, 6> MatVecMPOS_JCB_FP32<T>::get_tensor_shf() const {
    Eigen::Tensor<T, 6> tensor = get_tensor();
    return tensor.shuffle(tenx::array6{1, 2, 0, 4, 5, 3});
}

template<typename T>
Eigen::Tensor<T, 6> MatVecMPOS_JCB_FP32<T>::get_tensor_ene() const {
    if(mpos_B.size() == 1) {
        auto t_token = t_genMat->tic_token();
        eig::log->debug("Generating tensor");
    ...
    throw std::runtime_error("MatVecMPOS_JCB_FP32<T>::get_tensor_ene(): Not implemented for mpos.size() > 1");
}

template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixType MatVecMPOS_JCB_FP32<T>::get_matrix() const {
    return tenx::MatrixCast(get_tensor(), rows(), cols());
}
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixType MatVecMPOS_JCB_FP32<T>::get_matrix_shf() const {
    return tenx::MatrixCast(get_tensor_shf(), rows(), cols());
}
template<typename T>
typename MatVecMPOS_JCB_FP32<T>::MatrixType MatVecMPOS_JCB_FP32<T>::get_matrix_ene() const {
    return tenx::MatrixCast(get_tensor_ene(), rows(), cols());
}

template<typename T>
typename MatVecMPOS_JCB_FP32<T>::SparseType MatVecMPOS_JCB_FP32<T>::get_sparse_matrix() const {
    // Fill lower
...
template<typename T>
double MatVecMPOS_JCB_FP32<T>::get_sparsity() const {
    auto sp = get_sparse_matrix();
    auto n  = static_cast<double>(size_mps);
    return (static_cast<double>(sp.nonZeros()) * 2.0 - n) / (n * n);
}
template<typename T>
long MatVecMPOS_JCB_FP32<T>::get_non_zeros() const {
    auto sp = get_sparse_matrix();
    return sp.nonZeros();
}

template<typename T>
long MatVecMPOS_JCB_FP32<T>::get_jcbMaxBlockSize() const {
    return jcbMaxBlockSize;
}

template<typename T>
bool MatVecMPOS_JCB_FP32<T>::isReadyFactorOp() const {
    return readyFactorOp;
}
template<typename T>
bool MatVecMPOS_JCB_FP32<T>::isReadyShift() const {
    return readyShift;
}

// Explicit instantiations
template class MatVecMPOS_JCB_FP32<fp64>;
template class MatVecMPOS_JCB_FP32cx64>;

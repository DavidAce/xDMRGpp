...
template<typename T>
class MatVecMPOS_JCB_FP32 {
    static_assert(std::is_same_v<T, fp64> or std::is_same_v<T, cx64>);

    public:
    using Scalar     = T;
    using T32        = std::conditional_t<std::is_same_v<T, fp64>, fp32, std::complex<fp32>>;
    using T64        = std::conditional_t<std::is_same_v<T, fp64>, fp64, std::complex<fp64>>;
    ...
    using VectorT32  = Eigen::Matrix<T32, Eigen::Dynamic, 1>;
    using MatrixT32  = Eigen::Matrix<T32, Eigen::Dynamic, Eigen::Dynamic>;
    using SparseT32  = Eigen::SparseMatrix<T32>;
    using SparseT32R = Eigen::SparseMatrix<T32, Eigen::RowMajor>;
    ...
    VectorT32                               jcbDiagA, jcbDiagB; // The diagonals of matrices A and B for block jacobi preconditioning (for jcbMaxBlockSize == 1)
    VectorT32                               denseJcbDiagonal;   // The inverted diagonals used when jcBMaxBlockSize == 1
    std::vector<std::pair<long, SparseT32>> sparseJcbBlocks;    // inverted blocks for the block Jacobi preconditioner stored as sparse matrices
    std::vector<std::pair<long, MatrixT32>> denseJcbBlocks;     // inverted blocks for the block Jacobi preconditioner stored as dense matrices

    using BICGType = Eigen::BiCGSTAB<SparseT32R, Eigen::IncompleteLUT<T32>>;
    std::vector<std::tuple<long, std::unique_ptr<SparseT32R>, std::unique_ptr<BICGType>>> bicgstabJcbBlocks; // Solvers for the block Jacobi preconditioner

    // using CGType = Eigen::ConjugateGradient<SparseType, Eigen::Lower | Eigen::Upper, Eigen::SimplicialLLT<SparseType>>;
    using CGType = Eigen::ConjugateGradient<SparseT32, Eigen::Lower | Eigen::Upper, Eigen::IncompleteCholesky<T32, Eigen::Lower | Eigen::Upper>>;
    std::vector<std::tuple<long, std::unique_ptr<SparseT32>, std::unique_ptr<CGType>>> cgJcbBlocks; // Solvers for the block Jacobi preconditioner

    using LLTType = Eigen::LLT<MatrixT32, Eigen::Lower>;
    std::vector<std::tuple<long, std::unique_ptr<LLTType>>> lltJcbBlocks; // Solvers for the block Jacobi preconditioner

    using LDLTType = Eigen::LDLT<MatrixT32, Eigen::Lower>;
    std::vector<std::tuple<long, std::unique_ptr<LDLTType>>> ldltJcbBlocks; // Solvers for the block Jacobi preconditioner

    using LUType = Eigen::PartialPivLU<MatrixT32>;
    ...
    T32       get_matrix_element(long I, long J, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                                 const Eigen::Tensor<T, 3> &ENVR) const;
    VectorT32 get_diagonal_new(long offset, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                               const Eigen::Tensor<T, 3> &ENVR) const;
    MatrixT32 get_diagonal_block_old(long offset, long extent, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                                     const Eigen::Tensor<T, 3> &ENVR) const;
    MatrixT32 get_diagonal_block(long offset, long extent, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL,
                                 const Eigen::Tensor<T, 3> &ENVR) const;
    MatrixT32 get_diagonal_block(long offset, long extent, T32 shift, const std::vector<Eigen::Tensor<T, 4>> &MPOS_A, const Eigen::Tensor<T, 3> &ENVL_A,
    ...
    MatrixT32 get_diagonal_block_old(long offset, long extent, T32 shift, const std::vector<Eigen::Tensor<T, 4>> &MPOS_A, const Eigen::Tensor<T, 3> &ENVL_A,
                                     const Eigen::Tensor<T, 3> &ENVR_A, const std::vector<Eigen::Tensor<T, 4>> &MPOS_B, const Eigen::Tensor<T, 3> &ENVL_B,
                                     const Eigen::Tensor<T, 3> &ENVR_B) const;

    // VectorType get_diagonal_old(long offset) const;
    VectorT32 get_row(long row_idx, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const;
    VectorT32 get_col(long col_idx, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const;
    VectorT32 get_diagonal(long offset, const std::vector<Eigen::Tensor<T, 4>> &MPOS, const Eigen::Tensor<T, 3> &ENVL, const Eigen::Tensor<T, 3> &ENVR) const;
    ...
    public:
    MatVecMPOS_JCB_FP32() = default;
    template<typename EnvType>
    MatVecMPOS_JCB_FP32(const std::vector<std::reference_wrapper<const MpoSite>> &mpos, /*!< The Hamiltonian MPO's  */
    ...
    void CalcPc(T32 shift = 0.0);                         //  Calculates the diagonal or tridiagonal part of A
    void MultPc(T *mps_in_, T *mps_out, T32 shift = 0.0); //  Applies the preconditioner as the matrix-vector product x_out <- inv(A-sigma*I)*x_in.

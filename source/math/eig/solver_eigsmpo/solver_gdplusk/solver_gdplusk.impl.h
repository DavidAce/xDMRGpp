...
void solver_gdplusk<Scalar>::delete_blocks_from_left_until_orthogonal(const Eigen::Ref<const MatrixType> X, MatrixType &Y, MatrixType &HY,
                                                                      Eigen::Index maxBlocks, RealScalar threshold) {
...
template<typename Scalar>
void solver_gdplusk<Scalar>::selective_orthonormalize(const Eigen::Ref<const MatrixType> X,            // (N, xcols)
...
template<typename Scalar>
typename solver_gdplusk<Scalar>::MatrixType solver_gdplusk<Scalar>::get_Q_res(fMultPX_t MultPX) {
    assert(V.rows() == N);
    assert(V.cols() == b);
    ...
    Eigen::Index s1Blocks_old = s1Blocks;
    Eigen::Index s2Blocks_old = s2Blocks;

    auto get_total_blocks = [&]() { return wBlocks + sBlocks + s1Blocks + s2Blocks; };
    ...
    s1Blocks = 0;
    s2Blocks = 0;
    // Attempt to save iteration by adding residuals from H1 and H2
    if(S1.cols() == b) {
        s1Blocks = std::min(max_s1Blocks, s1Blocks_old + 1); // Add a S1 block
        if(s1Blocks > 0) eiglog->info("adding an s1 block");
    }
    if(S2.cols() == b) {
        s2Blocks = std::min(max_s2Blocks, s2Blocks_old + 1); // Add a S2 block
        if(s2Blocks > 0) eiglog->info("adding an s2 block");
    }
    ...
    while(N - V.cols() < get_total_blocks() * b) {
        ...
        if(s1Blocks > 0) { s1Blocks--; continue; }
        if(s2Blocks > 0) { s2Blocks--; continue; }
        if(sBlocks > 0) { sBlocks--; continue; }
        break; // If all are at min, break to avoid infinite loop
        /* clang-format on */
    }

    // orthogonalize S, S1 and S2 against V and Q
    auto breakdownTol                  = eps * 10 * std::max({RealScalar{1}, status.op_norm_estimate(algo)});
    auto orthogonalize_against_V_and_Q = [&](const Eigen::Ref<MatrixType> &matrix, Eigen::Index &numBlocks) -> MatrixType {
        if(numBlocks == 0 || matrix.cols() != b) return {};
        MatrixType matrix_ortho = matrix;
        VectorIdxT mask         = VectorIdxT::Ones(matrix.cols() / b);
        orthogonalize(V, matrix_ortho, breakdownTol, mask);
        // orthogonalize(Q, matrix_ortho, breakdownTol, mask);
        compress_cols(matrix_ortho, mask);
        return matrix_ortho;
    };

    // Absorb all blocks into a superblock and orthogonalize it against V and Q before preconditioning it
    MatrixType S_ortho(N, sBlocks + s1Blocks + s2Blocks);
    {
        Eigen::Index sOffset  = 0;
        Eigen::Index s1Offset = sBlocks;
        Eigen::Index s2Offset = sBlocks + s1Blocks;
        if(sBlocks > 0) S_ortho.middleCols(sOffset * b, b).noalias() = S;
        if(s1Blocks > 0) S_ortho.middleCols(s1Offset * b, b).noalias() = S1;
        if(s2Blocks > 0) S_ortho.middleCols(s2Offset * b, b).noalias() = S2;
        sBlocks  = sBlocks + s1Blocks + s2Blocks;
        s1Blocks = 0;
        s2Blocks = 0;
        if(dev_orthogonalization_before_preconditioning) {
            S_ortho = orthogonalize_against_V_and_Q(S_ortho, sBlocks);
            if(sBlocks != S_ortho.cols() / b) eiglog->debug("compressed sBlocks {} -> {}", sBlocks, S_ortho.cols() / b);
            sBlocks = S_ortho.cols() / b;
        }
    }
    ...
    if(wBlocks > 0) Q_res.middleCols(wOffset * b, b).noalias() = get_wBlock(MultPX);
    if(sBlocks > 0) Q_res.middleCols(sOffset * b, b).noalias() = get_sBlock(S_ortho, MultPX);

    // pick a relative breakdown tolerance:
    VectorIdxT active_block_mask = VectorIdxT::Ones(wBlocks + sBlocks);
    // orthonormalize(V, Q_res, breakdownTol, active_block_mask);
    orthonormalize(Q, Q_res, breakdownTol, active_block_mask);
    compress_cols(Q_res, active_block_mask);

    assert_allFinite(Q_res);
    assert_orthonormal(Q_res, orthTolQ);
    if constexpr(settings::print_q) eiglog->warn("Q_res after compression: \n{}\n", linalg::matrix::to_string(Q_res, 8));
    return Q_res;
}
...
void solver_gdplusk<Scalar>::build() {
    switch(algo) {
        ...
        case OptAlgo::XDMRG: {
            auto Q_res = get_Q_res([this](const Eigen::Ref<const MatrixType> &X, std::optional<const Eigen::Ref<const MatrixType>> iG) -> MatrixType {
...
template<typename Scalar>
void solver_gdplusk<Scalar>::build(MatrixType &Q_res, MatrixType &Q, MatrixType &HQ, fMultHX_t MultHX) {
    if(status.stopReason != StopReason::none) return;
    assert(Q_res.rows() == N);
    if(dev_append_extra_blocks_to_basis) {
        if(Q_res.cols() == 0 and s1Blocks + s2Blocks == 0 and dev_append_extra_blocks_to_basis) {
            max_s1Blocks = 1;
            max_s2Blocks = 1;
        } else {
            max_s1Blocks = 0;
    ...
    if(newBlocks > maxBasisBlocks or Q.cols() == 0 or Q_res.cols() == 0) {
        // (re)start
        if(Q_res.cols() == 0 and status.iter <= status.iter_last_restart + 2) {
        ...
        MatrixType Qold = Q;
        hhqr.compute(Q);
        auto rank = std::min(Q.rows(), Q.cols());
        Q         = hhqr.householderQ().setLength(rank) * MatrixType::Identity(N, rank);

        if(HQ.cols() != Q.cols()) {
            // Some block got removed by QR. This should not happen often (could happen when Q.cols() > N)
            HQ = MultHX(Q);
        } else {
            VectorReal D = (Qold.leftCols(rank).conjugate().array() * Q.array()).colwise().sum().real();
...
template<typename Scalar>
void solver_gdplusk<Scalar>::build(MatrixType &Q1_res, MatrixType &Q2_res, MatrixType &Q, MatrixType &H1Q, MatrixType &H2Q) {
    if(status.stopReason != StopReason::none) return;
    assert(Q1_res.rows() == N);
    assert(Q2_res.rows() == N);
    auto       breakdownTol = eps * 10 * std::max({RealScalar{1}, status.op_norm_estimate(algo)});
    VectorIdxT mask         = VectorIdxT::Ones(Q2_res.cols() / b);
    orthonormalize(Q1_res, Q2_res, breakdownTol, mask);
    compress_cols(Q2_res, mask);

    if(Q1_res.cols() + Q2_res.cols() == 0 and s1Blocks + s2Blocks == 0 and dev_append_extra_blocks_to_basis) {
        max_s1Blocks = 1;
        max_s2Blocks = 1;
    } else {
        max_s1Blocks = 0;
    ...
    if(newBlocks > maxBasisBlocks or Q.cols() == 0 or Q1_res.cols() == 0 or Q2_res.cols() == 0) {
        // (re)start
        if(Q1_res.cols() + Q2_res.cols() == 0 and status.iter <= status.iter_last_restart + 2) {
            // Failed to add a nonzero residual
            status.stopReason |= StopReason::saturated_basis;
            status.stopMessage.emplace_back("saturated basis: exhausted subspace search");
            return;
        ...
        if(H1Q.cols() != Q.cols() or H2Q.cols() != Q.cols()) {
            // Some block got removed by QR. This should not happen often (could happen when Q.cols() > N)
            H1Q = MultH1X(Q);
            H2Q = MultH1X(Q);
        } else {
...
template<typename Scalar>
void solver_gdplusk<Scalar>::build(MatrixType &Q_res, MatrixType &Q, MatrixType &H1Q, MatrixType &H2Q) {
    if(status.stopReason != StopReason::none) return;
    assert(Q_res.rows() == N);
...
template<typename Scalar>
void solver_gdplusk<Scalar>::set_maxLanczosResidualHistory(Eigen::Index w) {
    w = std::clamp<Eigen::Index>(w, 0, 1); // wBlocks are not required
    if(w != max_wBlocks) eiglog->trace("gdplusk: max_wBlocks = {}", max_wBlocks);
    max_wBlocks = w;
}

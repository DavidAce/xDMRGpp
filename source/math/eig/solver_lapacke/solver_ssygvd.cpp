...
int eig::solver::ssygvd(fp32 *matrixA, fp32 *matrixB, size_type L) {
    ...
    info = LAPACKE_ssygvd_work(LAPACK_COL_MAJOR, itype, jobz, uplo, n, matrixA, lda, matrixB, ldb, eigvals.data(), lwork_query, -1, liwork_query, -1);
    if(info < 0) throw std::runtime_error("LAPACKE_ssyevd_work query: info" + std::to_string(info));
    ...
    info = LAPACKE_ssygvd_work(LAPACK_COL_MAJOR, itype, jobz, uplo, n, matrixA, lda, matrixB, ldb, eigvals.data(), work.data(), lwork, iwork.data(), liwork);

    if(info < 0) throw std::runtime_error("LAPACKE_ssygvd_work: info" + std::to_string(info));

...
int eig::solver::wheevd_eigen(cx128 *matrix, size_type L) {
    eig::log->trace("Starting eig qsyevd");
    using Scalar                      = cx128;
    ...
    int  info       = 0;
    int  options    = config.compute_eigvecs == Vecs::ON ? Eigen::ComputeEigenvectors : Eigen::EigenvaluesOnly;
    auto t_prep     = std::chrono::high_resolution_clock::now();

    auto solver = Eigen::SelfAdjointEigenSolver<MatrixType>(matrix_map, options);
    if(solver.info() == Eigen::ComputationInfo::Success) {
        ...
        throw std::runtime_error("Eigen qsyevd failed with error: " + std::to_string(solver.info()));
    }

...
int eig::solver::dsyevr(fp64 *matrix /*!< gets destroyed */, size_type L, char range, int il, int iu, fp64 vl, fp64 vu) {
    ...
    info = DMRG_dsyevr_work(LAPACK_COL_MAJOR, jobz, range, 'U', lda, matrix, lda, vl, vu, il, iu, 2 * DMRG_dlamch('S'), &m_found, eigvals.data(),
                               eigvecs.data(), ldz, isuppz.data(), lwork_query, -1, iwork_query, -1);
    if(info < 0) throw std::runtime_error("DMRG_dsyevr_work query: info" + std::to_string(info));
    ...
    info = DMRG_dsyevr_work(LAPACK_COL_MAJOR, jobz, range, 'U', n, matrix, lda, vl, vu, il, iu, DMRG_dlamch('S'), &m_found, eigvals.data(),
                               eigvecs.data(), ldz, isuppz.data(), work.data(), lwork, iwork.data(), liwork);
    if(info < 0) throw std::runtime_error("DMRG_dsyevr_work: info" + std::to_string(info));
    /* From the MKL manual:
        abstol
    ...
    */
    ...
    if(info == 0) {
        ...
        // result.meta.residual_norms = std::vector<double>(2 * DMRG_dlamch('S'), m_found);

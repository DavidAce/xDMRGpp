...
int eig::solver::dsyevd(fp64 *matrix, size_type L) {
    ...
    info = DMRG_dsyevd_work(LAPACK_COL_MAJOR, jobz, uplo, n, matrix, n, eigvals.data(), lwork_query, -1, liwork_query, -1);
    if(info < 0) throw std::runtime_error("DMRG_dsyevd_work query: info" + std::to_string(info));
    ...
    info                       = DMRG_dsyevd_work(LAPACK_COL_MAJOR, jobz, uplo, n, matrix, n, eigvals.data(), work.data(), lwork, iwork.data(), liwork);
    if(info < 0) throw std::runtime_error("DMRG_dsyevd_work: info" + std::to_string(info));

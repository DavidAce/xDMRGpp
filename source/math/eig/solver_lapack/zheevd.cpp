...
int eig::solver::zheev(cx64 *matrix, size_type L) {
    ...
    auto rwork         = std::vector<fp64>(static_cast<size_t>(lrwork));

    int info =
        DMRG_zheev_work(LAPACK_COL_MAJOR, jobz, 'U', n, reinterpret_cast<lapack_complex_double *>(matrix), n, eigvals.data(), work_query, -1, rwork.data());
    ...
    auto work  = std::vector<cx64>(safe_cast<size_t>(lwork));

    info = DMRG_zheev_work(LAPACK_COL_MAJOR, jobz, 'U', n, reinterpret_cast<lapack_complex_double *>(matrix), n, eigvals.data(),
...
int eig::solver::zheevd(cx64 *matrix, size_type L) {
    ...
    fp64 rwork_query[1] = {};
    int  iwork_query[1] = {};
    int  info = DMRG_zheevd_work(LAPACK_COL_MAJOR, jobz, 'U', n, reinterpret_cast<lapack_complex_double *>(matrix), n, eigvals.data(), work_query, -1,
    ...
    auto work   = std::vector<cx64>(safe_cast<size_t>(lwork));
    auto rwork  = std::vector<fp64>(safe_cast<size_t>(lrwork));
    auto iwork  = std::vector<int>(safe_cast<size_t>(liwork));

    info = DMRG_zheevd_work(LAPACK_COL_MAJOR, jobz, 'U', n, reinterpret_cast<lapack_complex_double *>(matrix), n, eigvals.data(),

...
int eig::solver::dsygvx(fp64 *matrixA, fp64 *matrixB, size_type L, char range, int il, int iu, double vl, double vu) {
    ...
    info = DMRG_dsygvx_work(LAPACK_COL_MAJOR, itype, jobz, range, 'U', n, matrixA, lda, matrixB, ldb, vl, vu, il, iu, 2 * DMRG_dlamch('S'), &m_found,
                               eigvals.data(), eigvecs.data(), ldz, lwork_query, -1, iwork.data(), ifail.data());
    if(info < 0) throw std::runtime_error("DMRG_dsygvx_work query: info" + std::to_string(info));
    ...
    if(info < 0) throw std::runtime_error("DMRG_dsygvx_work: info" + std::to_string(info));
    /* From the MKL manual:
        abstol
    ...
    */
    ...
    if(info == 0) {
        ...
        throw std::runtime_error("LAPACK dsygvx failed with error: " + std::to_string(info));
    }
    return info;
}

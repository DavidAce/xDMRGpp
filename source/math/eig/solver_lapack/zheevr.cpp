...
int eig::solver::zheevr(cx64 *matrix /*!< gets destroyed */, size_type L, char range, int il, int iu, fp64 vl, fp64 vu) {
    ...
    std::vector<int> ifail(safe_cast<size_t>(L));
    ...
    if(config.compute_eigvecs == Vecs::ON)
        eigvecs.resize(safe_cast<size_t>(ldz) * safe_cast<size_t>(m_req)); // Docs claim ldz * m, but it segfaults when 'V' finds more than m eigvals

    info = DMRG_zheevr_work(LAPACK_COL_MAJOR, jobz, range, 'U', lda, matrix, lda, //
                               vl, vu, il, iu, 2 * DMRG_dlamch('S'),              //
                               &m_found, eigvals.data(), eigvecs.data(),             //
                               ldz, isuppz.data(), lwork_query, -1, rwork_query, -1, iwork_query, -1);
    if(info < 0) throw except::runtime_error("DMRG_zheevr_work query: info {}", info);
    ...
    std::vector<cx64>   work(safe_cast<size_t>(lwork));
    std::vector<double> rwork(safe_cast<size_t>(lrwork));
    ...
    eigvals.resize(safe_cast<size_t>(m_found));
    eigvecs.resize(safe_cast<size_t>(m_found) * safe_cast<size_t>(ldz));

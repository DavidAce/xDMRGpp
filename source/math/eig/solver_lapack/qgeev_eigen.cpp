...
int eig::solver::qgeev_eigen(fp128 *matrix, size_type L) {
    eig::log->trace("Starting eig qgeev_eigen");
    using Scalar     = fp128;
    ...
    auto solver = Eigen::EigenSolver<MatrixType>(matrix_map, config.compute_eigvecs == Vecs::ON);
    if(solver.info() == Eigen::ComputationInfo::Success) {
        ...
        if(config.compute_eigvecs == Vecs::ON) {
            auto &eigvecs = result.get_eigvecs<Cplx, Form::NSYM>();
            eigvecs.resize(static_cast<size_t>(L * L));
            auto evecs_map = Eigen::Map<MatrixCplx>(eigvecs.data(), L, L);
            evecs_map      = solver.eigenvectors();
        }
    } else {
        throw std::runtime_error("Eigen qgeev_eigen failed with error: " + std::to_string(solver.info()));
    }

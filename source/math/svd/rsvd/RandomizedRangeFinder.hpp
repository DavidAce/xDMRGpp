...
#include "StandardNormalRandom.hpp"
#include <Eigen/Dense>
...
namespace Rsvd {
    ...
    namespace Internal {
        ...
        struct RandomizedSubspaceIterations {
            ...
            /// of the approximate range of \f$ A \f$.
            static MatrixType compute(const MatrixType &a, Eigen::Index dim, unsigned int numIter, RandomEngineType &engine);
        };

        /// \brief Partial specialization for subspace iterations without a conditioner.
        template<typename MatrixType, typename RandomEngineType>
        struct RandomizedSubspaceIterations<MatrixType, RandomEngineType, SubspaceIterationConditioner::None> {
            static MatrixType compute(const MatrixType &a, const Eigen::Index dim, const unsigned int numIter, RandomEngineType &engine) {
                assert(numIter > 0);

                const auto numRows{a.rows()};
                const auto numCols{a.cols()};

                MatrixType tmpRows{a * standardNormalRandom<MatrixType, RandomEngineType>(numCols, dim, engine)};
        ...
        template<typename MatrixType, typename RandomEngineType>
        struct RandomizedSubspaceIterations<MatrixType, RandomEngineType, SubspaceIterationConditioner::Lu> {
            static MatrixType compute(const MatrixType &a, const Eigen::Index dim, const unsigned int numIter, RandomEngineType &engine) {
                assert(numIter > 0);
        ...
        /// for conditioning.
        template<typename MatrixType, typename RandomEngineType>
        struct RandomizedSubspaceIterations<MatrixType, RandomEngineType, SubspaceIterationConditioner::Mgs> {
            static MatrixType compute(const MatrixType &a, const Eigen::Index dim, const unsigned int numIter, RandomEngineType &engine) {
                assert(numIter > 0);
        ...
        /// conditioning.
        template<typename MatrixType, typename RandomEngineType>
        struct RandomizedSubspaceIterations<MatrixType, RandomEngineType, SubspaceIterationConditioner::Qr> {
            static MatrixType compute(const MatrixType &a, const Eigen::Index dim, const unsigned int numIter, RandomEngineType &engine) {
                assert(numIter > 0);

...
namespace svd::internal::lapack_wrappers {
    ...
    int cgesvj(Context<Scalar> &ctx) {
        ...
        int info = LAPACKE_cgesvj_work(LAPACK_COL_MAJOR, 'G', 'U', 'V', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.ldv, ctx.V.data(), ctx.ldv,
    ...
    template int cgesvj<cx32>(Context<cx32> &ctx);
}

...
namespace svd::internal::lapack_wrappers {
    ...
    int sgesdd(Context<Scalar> &ctx) {
        ...
        int info = DMRG_sgesdd_work(LAPACK_COL_MAJOR, 'S', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.U.data(), ctx.ldu,
        ...
        return DMRG_sgesdd_work(LAPACK_COL_MAJOR, 'S', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.U.data(), ctx.ldu,

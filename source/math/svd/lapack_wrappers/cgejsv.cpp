...
namespace svd::internal::lapack_wrappers {
    ...
    int cgejsv(Context<Scalar> &ctx) {
        ...
        int info = LAPACKE_cgejsv_work(LAPACK_COL_MAJOR, 'F', 'U', 'V', 'N', 'T', 'N', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.U.data(),
        ...
        rwork.resize(safe_cast<size_t>(std::max({7, ctx.colsA + 2 * ctx.rowsA, lrwork})));
        iwork.resize(safe_cast<size_t>(std::max({1, ctx.rowsA + ctx.colsA, liwork})));

        info = LAPACKE_cgejsv_work(LAPACK_COL_MAJOR, 'F', 'U', 'V', 'N', 'T', 'N', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.U.data(),
    ...
    template int cgejsv<cx32>(Context<cx32> &ctx);
}

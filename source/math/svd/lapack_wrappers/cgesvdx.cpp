...
namespace svd::internal::lapack_wrappers {
    ...
    int cgesvdx(Context<Scalar> &ctx) {
        ...
        int info = DMRG_cgesvdx_work(LAPACK_COL_MAJOR, 'V', 'V', sel.range, ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, sel.vl, sel.vu, sel.il, sel.iu,
        ...
        info = DMRG_cgesvdx_work(LAPACK_COL_MAJOR, 'V', 'V', 'V', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, sel.vl, sel.vu, sel.il, sel.iu, &ns,

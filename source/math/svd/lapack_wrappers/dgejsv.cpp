...
namespace svd::internal::lapack_wrappers {
    ...
    int dgejsv(Context<Scalar> &ctx) {
        ...
        int info = LAPACKE_dgejsv_work(LAPACK_COL_MAJOR, 'F', 'U', 'V', 'N', 'T', 'N', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.U.data(),
    ...
    template int dgejsv<fp64>(Context<fp64> &ctx);
}

...
namespace svd::internal::lapack_wrappers {
    ...
    int sgesvj(Context<Scalar> &ctx) {
        ...
        int info = DMRG_sgesvj_work(LAPACK_COL_MAJOR, 'G', 'U', 'V', ctx.rowsA, ctx.colsA, ctx.A.data(), ctx.lda, ctx.S.data(), ctx.ldv, ctx.V.data(), ctx.ldv,
                                    rwork.data(), lrwork);
    ...
    template int sgesvj<fp32>(Context<fp32> &ctx);
}
